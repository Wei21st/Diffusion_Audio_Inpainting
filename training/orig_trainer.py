...
class Trainer():
    ...
    def training_loop(self):
        ...
        while True:
            ...
            self.it += 1
    # def training_loop(self):
    #     # Get total iterations from config, or set a fixed number
    #     total_steps = self.args.exp.total_iters if hasattr(self.args.exp, 'total_iters') else 100000  # fallback default

    #     pbar = trange(self.it, total_steps, desc="Training", initial=self.it)
    #     for _ in pbar:
    #         # Accumulate gradients and train
    #         self.train_step()
    ...
    #         # Update tqdm bar with loss value if available
    #         if 'loss' in training_stats.default_collector.stats:
    #             try:
    #                 loss_val = training_stats.default_collector.mean('loss')
